...
def index_bill_file(conn: Connection, json_path: Path, archive_root: Path) -> None:
    ...
    # Per-file atomicity: sqlite3.Connection as context manager begins on
    # enter, commits on clean exit, rolls back on exception. If any INSERT
    # below raises (KeyError, OOM, etc.), the file's writes are reverted
    # rather than leaving the bills row without its sponsors/votes/FTS.
    with conn:
        conn.execute(
...
def index_event_file(conn: Connection, json_path: Path, archive_root: Path) -> None:
    ...
    rel_path = json_path.resolve().relative_to(archive_root.resolve()).as_posix()
    # Per-file atomicity — see index_bill_file. Rolls back any partial
    # event_items / events_fts writes if a downstream INSERT raises.
    with conn:
...
def index_person_file(conn: Connection, json_path: Path, archive_root: Path) -> None:
    with open(json_path, encoding="utf-8") as f:
        p = json.load(f)
    rel_path = json_path.resolve().relative_to(archive_root.resolve()).as_posix()
    # Per-file atomicity. Single-statement today, but a future add (e.g.
    # office assignments) would benefit from the same rollback boundary.
    with conn:
        conn.execute(
