...
def test_aggregate_events_groups_by_body_name(indexed_db):
    rows = aggregate_events(indexed_db, group_by=["body_name"])["results"]
...
def test_aggregate_events_event_year_returns_integer(indexed_db):
    rows = aggregate_events(indexed_db, group_by=["event_year"])["results"]
    assert rows
    assert all(isinstance(r["event_year"], int) for r in rows)


def test_aggregate_events_multi_dim_group_by(indexed_db):
    rows = aggregate_events(indexed_db, group_by=["body_name", "event_year"])["results"]
    assert rows
    assert all(
...
def test_aggregate_events_empty_group_by_is_grand_total(indexed_db):
    # Empty group_by collapses to a single grand-total row. The fixture has 1 event.
    out = aggregate_events(indexed_db, group_by=[])
    assert out["results"] == [{"count": 1}]
...
def test_aggregate_events_committee_filter_narrows(indexed_db):
    ...
    unfiltered = aggregate_events(indexed_db, group_by=["body_name"])["results"]
    assert unfiltered
    rows = aggregate_events(
...
def test_aggregate_events_event_month_excludes_null_date(indexed_db):
    ...
    indexed_db.commit()
    rows = aggregate_events(indexed_db, group_by=["event_month"])["results"]
    assert rows
